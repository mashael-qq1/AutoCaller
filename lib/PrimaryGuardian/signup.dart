...
class _PrimaryGuardianSignUpPageState extends State<PrimaryGuardianSignUpPage> {
  ...
  bool _isValidPhone(String phone) {
    return RegExp(r'^\d$').hasMatch(phone); // Starts with 05 & 10 digits
  }
  void _signUpGuardian() async {
    ...
    if (confirmPassword.isEmpty) {
      _showError("Please fill confirm password field.");return;}
      if (fullName.isEmpty) {
      _showError("Please enter a full name");
      return;
    }else if (!_isValidName(fullName)) {
      _showError(
      "Full name should be at least 3 characters and contain only letters.");return;}
      // Email validation
    if (email.isEmpty) {
  ...
  if (phone.isEmpty) {
    ...
    return; // Stop execution if phone number is empty
  }  else if (phone.length != 10) {
    _showError("Phone number must be 10 digits");
    return; // Stop execution if phone number isn't 10 digits long
  } else if (!RegExp(r'^\d{10}$').hasMatch(phone)) {
    _showError("Phone number can only contain numbers");
    return; // Stop execution if phone number contains non-numeric characters
  }
    ...
    if (password.isEmpty) {
      ...
      _showError("Password must not exceed 25 characters");
      return;
    } else if (!_hasMinLength || !_hasUpperCase || !_hasLowerCase || !_hasNumber || !_hasSpecialChar) {
      _showError("Please follow password requirements");
      return;
    }
    ...
    } catch (e) {_showError("Sign up Failed: ${e.toString()}");
    } finally {
      setState(() {
        _isLoading = false;
      });
